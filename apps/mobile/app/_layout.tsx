...
import { isWrongBrandOrg } from "@/lib/brand";
...
function RootLayout() {
  ...
  // The mobile-login API route now rejects a brand mismatch server-side
  // before a session is ever issued (see apps/web/app/api/auth/mobile-login),
  // so this only catches a session that predates that fix (already persisted
  // on disk from an old app version). Sign it out quietly — no alert, so we
  // never tell the device there's a second app it doesn't have access to.
  useEffect(() => {
    if (!session) return;
    getOrganizationInfo()
      .then((info) => {
